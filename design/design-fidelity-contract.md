...
> **Source of truth:** `design/styles.css` (Claude Design export), read directly — NOT reconstructed. These are the reference's *resolved* values.
>
> **How to use:** feed these numbers into `/specify` AC-13 as explicit computed-style targets, and into CT `getComputedStyle` assertions. **Copy the VALUES, never the CODE.** `var(--token)` ⇒ assert *resolves-to-token*; `color-mix(...)` ⇒ assert at token level.
>
