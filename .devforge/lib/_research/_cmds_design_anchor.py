"""Design-anchor capture command handler for research_helper (plan 53 Phase 1).

One setter: set-design-anchor. Captures design intent {kind, file, selectors}
into research-state.json (memo.design_anchor) for finalize-handoff to emit
...
53 D3/D5) -- unlike the 6 RUBRIC_DIMENSIONS, it does NOT participate in
symptom-coverage / symptom-finalize gating. --state mirrors the sibling
rubric setters' CLI shape for consistency but carries no gating semantics
here.
"""
...
def cmd_set_design_anchor(args: argparse.Namespace) -> int:
    ...
    if not ds.valid:
        return _die(
            "set-design-anchor: malformed --value {0!r}; expected "
    ...
    try:
        ...
        return _die(
            "set-design-anchor: --selectors is not valid JSON: {0}".format(err),
            code=2,
        )
    if not isinstance(decoded, list):
        return _die(
            "set-design-anchor: --selectors must decode to a JSON array, "
    ...
    for item in decoded:
        if not isinstance(item, str):
            return _die(
                "set-design-anchor: --selectors items must be strings, "
    ...
    try:
        ...
        return _die("set-design-anchor: {0}".format(err))
    return 0
