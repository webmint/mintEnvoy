"""Session state for review_helper.

Owns the ReviewState schema, path helper, and atomic read/write.

/review is a per-feature command. State is stored alongside the feature's
other artifacts at:

    <feature_dir>/review-state.json
...
`_pr_review/_state.py`, which scopes state to the per-entity directory
rather than a global singleton like `_audit`'s `audits/.state.json`.

Precedent followed: `_pr_review/_state.py` (per-entity scoped state) —
...
_STATE_FILENAME = "review-state.json"


@dataclass
class ReviewState:
    """Per-feature review session state stored at <feature_dir>/review-state.json.
...
def read_state(path: str) -> Optional[ReviewState]:
    """Read ReviewState from a JSON file at path.
    ...
    """
    known = {f.name for f in dataclasses.fields(ReviewState)}
    try:
    ...
    return ReviewState(**filtered)


def write_state(path: str, state: ReviewState) -> None:
...
def flip_phase(
    ...
    Returns the updated ReviewState.
    """
