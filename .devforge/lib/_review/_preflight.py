"""Pure preflight function for review_helper.
...
Sentinel set is intentionally identical to _audit/_preflight.py so the two
helpers enforce the same gate on the same markers.

Setup-chain artefacts checked (same as /audit's preflight):
...
# These are the exact strings checked by _audit/_preflight.py; any change
# here must be reflected there (and vice versa) to keep parity.
...
# Setup-chain artefacts that must exist for /review to run.
# Parallel to /audit's preflight — same four-command chain.
...
def preflight_context(workspace_root: str) -> Dict:
    ...
    try:
        ...
        for line in claude_lines:
            ...
            # Mirrors _audit/_preflight.py's logic exactly.
            # Known limitation (shared with _audit/_preflight): a path value
            # containing a colon — e.g. a Windows drive letter like C:\Users\me
            # — is truncated to the part after the last colon (\Users\me).
            # Forge installs on Windows are uncommon; accepted.
