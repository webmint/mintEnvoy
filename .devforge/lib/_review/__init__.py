"""Internal package for review_helper (the /review command's mechanical work).

Submodules are underscore-prefixed. External callers invoke via the POSIX
launcher `review_helper` or `review_helper.main`.
...
Phase 1 ships 2 verbs (scaffold):
  check-status-and-flip  — read or update per-feature review state
  preflight              — gate on 4-command setup chain + populated constitution

Later phases add scope/dispatch/refutation/report verbs via _SUBCOMMAND_REGISTRY
