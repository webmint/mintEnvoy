...
- **[Task 001 / 018-code-editor-extract]**: create-codeeditor-molecule — completed. _(Task 001)_
- **[Task 002 / 018-code-editor-extract]**: wire-bodyeditor-consumer — completed. _(Task 002)_
- **[Task 003 / 018-code-editor-extract]**: codeeditor-fidelity-ct — completed. _(Task 003)_
- **[Task 004 / 018-code-editor-extract]**: update-bodyeditor-ct — completed. _(Task 004)_
- **[Task 005 / 018-code-editor-extract]**: docs-codeeditor-molecule — completed. _(Task 005)_
- **[Task 001 / 019-code-editor-toggle]**: Rebuild CodeEditor as edit-preview toggle molecule — completed. _(Task 001)_
- **[Task 002 / 019-code-editor-toggle]**: Add edit/preview toggle + render-phase reset to BodyEditor — completed. _(Task 002)_
- **[Task 003 / 019-code-editor-toggle]**: CodeEditor CT + stories (toggle/caret/focus/recolor) — completed. _(Task 003)_
- **[Task 004 / 019-code-editor-toggle]**: BodyEditor CT + stories (toggle/AC-6/focus) — completed. _(Task 004)_
- **[Task 005 / 019-code-editor-toggle]**: Document CodeEditor edit/preview toggle in architecture.md — completed. _(Task 005)_
...
## 2026-07-19 — 018-code-editor-extract: APPROVED; live-verify substitutes for a broken CT harness
Feature extracted the raw code editor from BodyEditor into a CodeEditor molecule. The Playwright CT harness is broken locally (organism/molecule story fixtures fail "cannot be mounted" — see memory ct-organism-fixtures-cannot-mount-locally), so no CT could RUN. Resolution that worked: verify tests STATICALLY (typecheck/lint/build) + panel-review the test code, AND prove the load-bearing runtime ACs LIVE via Chrome DevTools MCP against `electron-vite dev` on :5173. AC-4 (same-tab mode-switch scroll+caret preservation), AC-6 (resetKey reset), AC-11 (onChange→store→highlight) were all live-confirmed, overriding the memory-flagged "display:none loses scrollTop" concern (that was feature-015 panel-divs, NOT this textarea/pre — Chromium preserves it here). /verify ac_verification_mode=tests => code-reading; TEST_COMMANDS=N/A so regression gate inconclusive (not gated). Hygiene flagged 34 leftover_artifacts (ALL false positives — explanatory // comments in comment-dense CT files) + 1 scope-creep (CodeEditor.stories.tsx, the justified Playwright-CT stories-module deviation) — compute-verdict correctly folded both as ADVISORY, non-blocking. Took 4 /review passes + 3 /fix cycles to converge (later passes only re-raised already-dismissed SCROLL_BODY-dup / onChange-memo preferences).

## 2026-07-22 — 019-code-editor-toggle: APPROVED; live-MCP testing caught what the broken CT harness + per-task panels could not

Verify APPROVED 23/23 ACs (code-read, `tests` mode; suite not runnable — CT harness mount break). Feature-level lessons:
- **Live Chrome DevTools MCP against the running app (`npm run dev:audit`, CDP :50757) was load-bearing.** Four real UX defects surfaced ONLY by driving the app — empty-body dead-zone click-to-edit, edit-mode textarea clipping lines past line 2 (`rows` stuck at intrinsic 2), the focus ring clipped 3px past the window edge (flush-to-edge editor), and the "green border not in design" (focus-within ring on every mouse click). None were caught by the per-task `/implement` panels or the (unrunnable) CT harness. Reinforces [[design-auditor-clean-can-miss-layout]] and the 018 live-verify substitution.
- **`/review` + `/verify` finder subagents were unstable on this feature** (deaths on large embedded briefs, stalls on file reads). Fix: COMPACT source-only briefs (≤~850 lines) or embed the full small source and forbid extra reads; re-fan a died/stalled reviewer with the exact code embedded. See [[ct-subagent-stalls-even-on-reads]].
- **Fix borderless-editor answer = Postman/CodeMirror parity:** a text field's caret is its WCAG-2.4.7 focus indicator, so drop the container `:focus-within` ring in edit mode; keep a `:focus-visible` keyboard ring only on the caret-less preview `<pre>`. And toggle a11y: `aria-pressed` + NO `aria-label` (visible text is the accessible name → WCAG 2.5.3 Label-in-Name; a stable "Edit mode" label would violate it in the "Preview" state).
- Open (deferred): the 12px right-gutter (Postman-parity, user-approved) deviates from `design/reference.html` (`padding-right:0`); the 019 design-manifest is the minimal `{route,pairs}` shape with no `deviate_reason` field, so declare it by updating the reference or accept the recorded review.md deviation.
