...
- **Carry the design binding's testid (framework build requirement, not a constitution rule).** When the feature has a design binding (`specs/[feature]/design-manifest.json`), set a stable web test-id attribute (e.g. `data-testid`) equal to the binding pair's `built_testid` on the region or element you build, so the review-time design-fidelity check locates that built element by its stable testid rather than by fuzzy DOM matching. The testid goes on the containing region — and on any element the binding pairs individually — never as a per-element anchor mirroring reference ids.
