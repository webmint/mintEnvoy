...
2. [Medium] src/renderer/src/components/molecules/**tests**/Dropdown.test.tsx:349 — Cross-task icon-rendering path gap — the items-array `icon` prop route through Dropdown (Task 7) to Icon (Task 3) is untested; only the children-API `DropdownItem icon` path is exercised [Certain]
...
### src/renderer/src/components/molecules/**tests**/Dropdown.test.tsx
...
- [F-003] [Medium] :349 — Cross-task icon-rendering path gap — the items-array `icon` prop route through Dropdown (Task 7) to Icon (Task 3) is untested; only the children-API `DropdownItem icon` path is exercised [Certain]
  Severity: Medium
  File: src/renderer/src/components/molecules/**tests**/Dropdown.test.tsx
  Line: 349
...
- [D-003] [Medium] src/renderer/src/components/**tests**/PrimitivesDemo.test.tsx:29 — Cross-task shared-store leakage — PrimitivesDemo test renders toastStore consumers (Task 9) without resetting the module-level store owned by Task 4, while Toast.test.tsx (Task 5) does reset it
