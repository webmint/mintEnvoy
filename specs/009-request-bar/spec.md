...
| Unit + component tests            | src/renderer/src/components/organisms/**tests**/RequestBar.test.tsx, src/renderer/src/components/organisms/**tests**/RequestBar.ct.tsx, src/renderer/src/lib/**tests**/tabsStore.test.ts | Create new / extend — RequestBar unit (trim guard, method-switch independence, Save dirty/no-op) + CT (layout fidelity, disabled Send, ⌘↵/⌘S, method-dropdown dismiss two-step gate, per-tab isolation); tabsStore.test.ts gains updateActiveSpec cases (dirty-on-change, no-op no-flip, per-tab isolation) |
...
- [x] **AC-30**: The RequestBar and tabsStore test suites shall pass.
  > Verification: npx vitest run src/renderer/src/components/organisms/**tests**/RequestBar.test.tsx src/renderer/src/lib/**tests**/tabsStore.test.ts
...
- Must follow: design/reference.html is a look/behavior reference only — never reproduce its markup or generated cruft (data-om-\*, \_\_OmT wrappers, inline styles, tweaks-panel); rebuild with semantic class names from each element's role.
...
- Must follow constitution §3.4: Gate behavior with Vitest + Testing Library (jsdom) and Playwright CT for focus/keyboard fidelity, co-located under **tests**/ split .test.tsx/.ct.tsx.
