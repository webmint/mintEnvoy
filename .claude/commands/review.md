...
This sub-step adds the runtime design-fidelity check the 5-finder ensemble does not perform. It DISPATCHES `design-auditor` ONLY when BOTH a `design/reference.html` (the workspace-root design artifact the feature's UI implements against) AND a `specs/[feature]/design-manifest.json` **binding** (`{route, pairs}` — the built-side wiring `/breakdown` authors: the render route plus the anchor-selector↔built-testid pairs) exist. `design-auditor` reads that binding together with the feature's `specs/[feature]/design-anchor.json` **anchor** (`{kind, file, selectors}` — the captured design intent) and the built app, and reports a deterministic design-fidelity result — NOT-COVERED / CLEAN / DEFECT (an always-on sanity floor of overflow / clipping / font-not-loaded, plus anchor-gated value + geometry fidelity when the anchor is present). When `design/reference.html` is absent, this feature is not UI-against-a-reference work — skip the sub-step SILENTLY. When `design/reference.html` is present but the binding is absent, the design-fidelity intake gate (`/breakdown`) was skipped — emit a loud WARN naming the void runtime-conformance guarantee (the runtime check reports NOT-COVERED for this feature — this is never a silent pass), then skip the dispatch (the mechanical three-branch guard below). Only when both are present does this sub-step dispatch `design-auditor`. `design-auditor` runs the RUNTIME conformance check only; the STATIC provenance gate (no hardcoded color literals / no `var(--x, <literal>)` fallbacks) is a separate write-time gate at `/implement` and is NOT re-run here. Its deterministic design-fidelity result — the coverage verdict, the findings it measured, and the non-gating advisory — is embedded VERBATIM as a distinct `## Design Fidelity` section in the report, ENTIRELY OUTSIDE the refutation partition: a probe measurement is not a hypothesis to cross-examine, so it is never parsed into the ensemble buckets, never counted in any confirmed/dismissed total, and never routed to a refuter (wired via `render-report --design-section` in PHASE 4).

**Existence guard.** Check `design/reference.html` and the `specs/[feature]/design-manifest.json` binding mechanically (substitute `[feature]` with the resolved feature dir):

```bash
test -f design/reference.html && echo HAS_REF
test -f specs/[feature]/design-manifest.json && echo HAS_BINDING
```
