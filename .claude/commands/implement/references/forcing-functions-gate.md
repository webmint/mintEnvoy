...
`verify-design-tokens` is the static, write-time PROVENANCE half of the framework's design-fidelity mechanism, backing the constitution's Design Fidelity principle in its Code Quality Standards material. Over component styles it runs four manifest-independent checks: no hardcoded hex / `rgb()` / `hsl()` / named colors; no `var(--x, <literal>)` fallbacks; an undefined token FAILS LOUDLY rather than silently rendering a fallback; every interactive element declares both `:hover` and `:focus-visible`.

It is config-gated like every other rule — when `forcing_functions.design_token_provenance.enabled` is `true` the gate dispatches it; when the rule block is absent or `enabled: false` it is skipped (no-op). The rule block, when present, carries the config field `/constitute` writes today — `token_source_css`, the design token source. An older config may also carry a legacy `manifest_path` field, but it is inert: retained only for config back-compat and never read by `verify-design-tokens` (the manifest-consuming check it once fed was retired). All four checks are manifest-independent: `verify-design-tokens` reads no per-feature design manifest, so the orchestrator passes no manifest argument and the helper reads everything it needs from `.devforge/constitute.json`. The block's shape is owned by `constitute_helper`'s `set-forcing-functions` setter (`_constitute._forcing_functions._setters`); writing it into `.devforge/constitute.json` is the forcing-functions config-capture's job, so until that capture populates an enabled `design_token_provenance` block this gate skips the rule as a no-op.
